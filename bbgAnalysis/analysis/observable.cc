...
void observable::AnalysisR3(int n_entries, bool inclusive) {
  std::cout << "########## AnalysisR3() ##########" << std::endl;
  //Define Observables @PS & Reco levels
  //1st index:STEP 0=NO Selection/1=Cut Radiative return/2=Extract clean bbbar@Reco/3=STEP2+costheta<0.8
  //2nd index:Each ycut points (be obtained from the Ntuple)
  double R3b_ps[4][50];   //The Number of 3b-jets event @PS level
  double R3l_ps[4][50];
  double R3b_reco[4][50];
  double R3l_reco[4][50];
  double Rall_b_ps[4];    //The Number of All bbbar jets event @PS level
  double Rall_l_ps[4];
  double Rall_b_reco[4];
  double Rall_l_reco[4];
  double eR3b_ps[4][50];  //The error of upper fraction(b-jet ratio) @PS level
  double eR3l_ps[4][50];
  double eR3b_reco[4][50];
  double eR3l_reco[4][50];
  double R3bl_ps[4][50];   //Observable @PS level
  double R3bl_reco[4][50];
  double eR3bl_ps[4][50];  //The error of observable @PS level
  double eR3bl_reco[4][50];

  double ycut_array[50];//ycut values which is obtained from the Ntuple(Branch Name:ycut[50])
  ...
  for(Long64_t jentry=0; jentry<nentries; jentry++) {
    ...
    if(ISR_E>35) continue; //Radiative return cut 
    ...
    if(fabs(mc_quark_pdg[0])==5) {
      ...
      for(int iycut=0; iycut<50; iycut++) {
        ycut_njet -> Fill(mc_quark_ps_ycut[iycut], mc_quark_ps_njets_ycut[iycut]);
        h1_ycut -> Fill(mc_quark_ps_ycut[iycut]);
        h1_njet -> Fill(mc_quark_ps_njets_ycut[iycut]);

        bool njets_condition = false;
        //inclusive=true  : Inclusive Jet-Clustering(Fix y23)
        //inclusive=false : Exclusive Jet-Clustering(Fix Jet Number)
  ...
  for(int i=0; i<4; i++) {
    for(int j=0; j<50; j++) {
      ...
      if(Rall_b_reco[i]>0 && R3b_reco[i][j]>0) {
      ...
      if(Rall_l_reco[i]>0 && R3l_reco[i][j]>0) {
  ...
  TString filename="output_y23_cambridge_inclusive.root";
  if(inclusive==false) filename="output_y23_cambridge_exclusive.root";
...
void observable::AnalysisR3_cambridge(int n_entries) {
  ...
  double R3b_reco[4][50];
  double R3l_reco[4][50];
  double R3b_hadron[4][50];
  double R3l_hadron[4][50];
  double Rall_b_ps[4];    //The Number of All bbbar jets event @PS level
  double Rall_l_ps[4];
  double Rall_b_reco[4];
  double Rall_l_reco[4];
  double Rall_b_hadron[4];
  double Rall_l_hadron[4];
  double eR3b_ps[4][50];  //The error of upper fraction(b-jet ratio) @PS level
  double eR3l_ps[4][50];
  double eR3b_reco[4][50];
  double eR3l_reco[4][50];
  double eR3b_hadron[4][50];
  double eR3l_hadron[4][50];
  double R3bl_ps[4][50];   //Observable @PS level
  double R3bl_reco[4][50];
  double R3bl_hadron[4][50];
  double eR3bl_ps[4][50];  //The error of observable @PS level
  double eR3bl_reco[4][50];
  double eR3bl_hadron[4][50];

  double ycut_array[50];//ycut values which is obtained from the Ntuple(Branch Name:ycut[50])
  double eycut_array[50];//It's completely zero. We need it formally for drawing TGraphErrors
...
void observable::AnalysisR3_y23(int n_entries) {
  ...
  for(Long64_t jentry=0; jentry<nentries; jentry++) {
    ...
    if(fabs(mc_quark_pdg[0])==5) {
      ...
      if(PreSelection(2,35)==true && costheta_reco0<0.8) {
        Rall_b_ps[3]++;
        Rall_b_reco[3]++;
      ...
      if(mc_quark_ps_y23*jet_R_norm>0.01) {
        R3b_ps[0]++;
        if(PreSelection(1,35)==true) R3b_ps[1]++;
        if(PreSelection(2,35)==true) R3b_ps[2]++;
        if(PreSelection(2,35)==true && costheta_reco0<0.8) R3b_ps[3]++;
      }
      //Count jet events @Reco level
      if(y23*jet_R_norm>0.01) {
        R3b_reco[0]++;
        if(PreSelection(1,35)==true) R3b_reco[1]++;
        if(PreSelection(2,35)==true) R3b_reco[2]++;
        if(PreSelection(2,35)==true && costheta_reco0<0.8) R3b_reco[3]++;
      }
    }//bottom if
    ...
    if(fabs(mc_quark_pdg[0])<4) {
      ...
      if(PreSelection(3,35)==true && costheta_reco0<0.8) {
        Rall_l_ps[3]++;
        Rall_l_reco[3]++;
      ...
      if(mc_quark_ps_y23*jet_R_norm>0.01) {
        R3l_ps[0]++;
        if(PreSelection(1,35)==true) R3l_ps[1]++;
        if(PreSelection(3,35)==true) R3l_ps[2]++;
        if(PreSelection(3,35)==true && costheta_reco0<0.8) R3l_ps[3]++;
      }
      //Count jet events @Reco level
      if(y23*jet_R_norm>0.01) {
        R3l_reco[0]++;
        if(PreSelection(1,35)==true) R3l_reco[1]++;
        if(PreSelection(3,35)==true) R3l_reco[2]++;
        if(PreSelection(3,35)==true && costheta_reco0<0.8) R3l_reco[3]++;
      }
    }//uds if
  }//event loop
