...
void Correction() {
    ...
    canvas1->cd();
    TLegend* legend1 = new TLegend(0.15,0.75,0.4,0.88);
    ...
    legend1 -> AddEntry(g_cdet_bl, "C^{det}", "p");
    legend1 -> AddEntry(g_cdet_con_bl, "C^{'det}", "p");
    ...
    TText* t = new TText(0.2,0.7,"Cambridge");
    t -> SetNDC(1);
    t -> SetTextSize(0.05);
    t -> Draw();
