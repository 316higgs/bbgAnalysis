...
void R3bl_plots() {
  ...
  TFile* f = new TFile("R3_cambridge_signal.root");
  ...
  TCanvas* c = new TCanvas("results", "results", 800, 800);
  c -> cd(1);
  TLegend* leg = new TLegend(0.3, 0.2, 0.65, 0.35);
  ...
  for(int i=0; i<3; i++) {
    ...
    R3bl_ps[i]->GetYaxis()->SetRangeUser(0.7,1.4);
    R3bl_ps[i]->GetXaxis()->SetTitle("y_{cut}");
    //if(i=0) R3bl_ps[i]->Draw("");
    //if(i=1; i<3; i++) R3bl_ps[i]->Draw("SAME");
  }
  R3bl_ps[0]->SetLineColor(kGray+3);
  R3bl_ps[0]->SetMarkerStyle(0);
  R3bl_ps[0]->Draw("");
  R3bl_reco[2]->SetLineColor(kRed);
  R3bl_reco[2]->Draw("SAME");

  leg->AddEntry(R3bl_ps[0], "PS - Cambridge -> Selection step0", "L");
  leg->AddEntry(R3bl_reco[2], "RECO - Cambridge -> Selection step2", "L");
  leg->SetFillStyle(0);
  leg->SetLineWidth(0);
  leg->SetLineColor(0);
  leg->SetBorderSize(0);
